<!DOCTYPE html>
<html>
<head>
		<meta charset = "UTF-8">
		<title>SMART ICT</title>
		<script src="js/jquery.js"></script>
		<link rel="stylesheet" href="fotorama.css">
		<script src="fotorama.js"></script>
		<link rel="stylesheet" href="styles.css">
		<link rel="stylesheet" href="css/bootstrap.css">
		<link rel="stylesheet" href="css/bootstrap-theme.css">
		<link rel="stylesheet" type="text/css" href="css/responsive.css" media="screen">
		<meta name="viewport" content="width=divice-width, initial-scale=1" charset="UTF-8">
</head>
<body>
			
		<?php
 	include 'Nav.php';
		?> 
		
		
	<div class="panel panel-primary" >...</div>
	<div class="panel panel"></div>
	<h1 class="text-center text-primary ">Contact us</h1>
	<br><br><br>
	
	
	<nav class="container text-center">
		<div class="col-xs-12 col-md-12">
			<div class="row">
			<address>
			  <strong>SMART ICT Co.,Ltd.</strong><br>
			  	92/375 หมู่ที่ 5 ตำบลบางรักน้อย <br>
			  	 อำเภอเมืองนนทบุรี จังหวัด นนทบุรี 11000<br>
			</address>
			<address>
			  <strong>Telephone Number</strong><br>
			  082-7279850
			</address>
			<address>
			  <strong>Fax</strong><br>
			  02-926-1419-4
			</address>
			<address>
			  <strong>E-MAIL</strong><br>
			  <a href="mailto:#">siriporn.s@smartict-th.com</a>
			</address>
			<address>
			  <strong>Office Hour </strong><br>
			  09.00 - 17.00 Monday-Friday
			</address>
			</div>
			
			<h2 class="text-danger">Map</h2>
			
			<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m8!1m3!1d968.3058932805968!2d100.45541002146527!3d13.88557579234712!3m2!1i1024!2i768!4f13.1!4m7!1i0!3e0!4m0!4m3!3m2!1d13.8857976!2d100.4550828!5e0!3m2!1sth!2sus!4v1426582565286" 
			width="600" height="450"  style="border:0">
			</iframe>
			
		</div>	
	</nav>


	<footer class="container-fluid">
				<div class="alert alert-info">
				<div class="row">
					<div class="col-sm-6 col-xs-6">
						<nav class="nav navbar-nav navbar-left">&copy;Copyright SMART ICT Co.,Ltd.</nav>
					</div>
					<div class="col-sm-6 col-xs-6">
					<nav class="nav navbar-nav navbar-right">
						<a href="#">About Us</a> | <a href="#">Help</a> |
						<a href="#">SiteMap</a>
					</nav>
					</div>
				</div>
				</div>
			</footer>
	<script src="js/jquery-1.10.2.js"></script>
	<script src="js/bootstrap.min.js"></script>
	<script src="//maps.google.com/maps/api/js?sensor=true"></script>
	<script src="js/main.js"></script>	
			
	</body>
</html>