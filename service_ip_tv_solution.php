<!DOCTYPE html>
<html>
<head>
		<meta charset = "UTF-8">
		<title>SMART ICT</title>
		<script src="js/jquery.js"></script>
		<link rel="stylesheet" href="fotorama.css">
		<script src="fotorama.js"></script>
		<link rel="stylesheet" href="styles.css">
		<link rel="stylesheet" href="css/bootstrap.css">
		<link rel="stylesheet" href="css/bootstrap-theme.css">
		<link rel="stylesheet" type="text/css" href="css/responsive.css" media="screen">
		<meta name="viewport" content="width=divice-width, initial-scale=1" charset="UTF-8">
</head>
<body>
			
		<?php
 	include 'Nav.php';
		?> 
		
		
	<div class="panel panel-primary" >...</div>
	<div class="panel panel"></div>
		
	<div class="container">
		<nav class="panel panel-primary" >
				<h1 class="text-center text-primary">IP TV Solution</h1>
				<h3 class="text-info">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;IP TV Solution</h3>
				<div class="p">
					<p class="big">บริการด้านจัดทำระบบ IPTV ดู TV ผ่านระบบ IP ที่มีคามสามรถต่างจาก TV ทั่วไป </p>
				</div>
				<div align="center">
					<img src="ServicePic/iptv1.png" width="55%" height="55%">
					<br>
					<br>
					<br>
				</div>
		</nav>
			<div class="panel panel-primary">
				<div align="center">
					<img src="ServicePic/iptv2.png" width="55%" height="55%">
					<br>
					<br>
					<br>
				</div>
			</div>
	</div>
	
	
	
	
				<?php
			 	include 'footer.php';
				?> 
		
	
	
			
			
			
	<script src="js/jquery-1.10.2.js"></script>
	<script src="js/bootstrap.min.js"></script>
	<script src="//maps.google.com/maps/api/js?sensor=true"></script>
	<script src="js/main.js"></script>	
			
	</body>
</html>